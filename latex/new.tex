...
\begin{itemize}
    ...
    \item Judges' scores $\mathbf{S} = (S_1, S_2, \ldots, S_n)$, where $S_i \in [4, 40]$
    % 淘汰结果E，即被淘汰选手的索引
...
\end{equation}
% 其中R_i^S和R_i^π分别是选手i按评委分数和投票的排名
where $R_i^{S}$ and $R_i^{\pi}$ are the contestant's ranks by judges' scores and votes respectively.

% 百分比法（第3-27季）：
\textbf{Percentage Method} (Seasons 3--27):
% 综合得分 = 评委分数百分比 + 投票百分比，得分最低者被淘汰
\begin{equation}
C_i^{\text{pct}} = \frac{S_i}{\sum_j S_j} + \pi_i, \quad E = \arg\min_i C_i^{\text{pct}}
\end{equation}

% 从数学上看，给定淘汰结果E，存在无穷多个可能产生该结果的投票分布。
Mathematically, given an elimination outcome $E$, infinitely many vote distributions
...
% 先验权重向量θ设计为加权混合：
The weight vector $\boldsymbol{\theta}$ is designed as a weighted mixture:
\begin{equation}
